...
/**
 * Inspects the various properties and parts of an MDN wrapped in a S/MIME
 * message.
 * <p>
 * This class is typically used by the sender of the business message, which
 * will receive an MDN from the receiving party.
 * <p>
 * Part 0 : multipart/report; report-type=disposition-notification; 0 : Sub part
 * 0 : text/plain 0 : Sub part 1 : message/disposition-notification 0 : Sub part
 * x : will not be used by Oxalis Part 1 : application/pkcs7-signature;
...
public class MdnMimeMessageInspector {
	...
	/**
	 * The multipart/report should contain both a text/plain part with textual
	 * information and a message/disposition-notification part that should be
	 * examined for error/failure/warning.
	 */
	...
	/**
	 * We assume that the first text/plain part is the one containing any textual
	 * information.
	 */
	...
	/**
	 * We search for the first message/disposition-notification part. If we don't
	 * find one of that type we assume that part 2 is the right one.
	 */
	public BodyPart getMessageDispositionNotificationPart() {
		...
		if (bp == null)
			bp = getBodyPartAt(1); // the second part should be machine readable
		return bp;
	}
	...
	public Map<String, String> getMdnFields() {
		...
		try {
			...
			//
			// look for base64 transfer encoded MDN's (when Content-Transfer-Encoding is
			// present)
			//
			// Content-Type: message/disposition-notification
			// Content-Transfer-Encoding: base64
			//
			// "Content-Transfer-Encoding not used in HTTP transport Because HTTP, unlike
			// SMTP,
	...
	public boolean isOkOrWarning(Mic outboundMic) throws NoSuchAlgorithmException {
		 ...
		 * @author Manuel Gozzi
		 */
		Set<String> keySet = mdnFields.keySet();
		keySet.stream().forEach(key -> log.info("MDN field: {} - {}", key, mdnFields.get(key)));

		/*
		 * --------_=_NextPart_001_B096DD27.9007A6CE Content-Type:
		 * message/disposition-notification
		 *
		 * Reporting-UA: AS2 eefacta Server (unimaze.com) Original-Recipient: rfc822;
		 * SMP_2000000005 Final-Recipient: rfc822; SMP_2000000005 Original-Message-ID:
		 * a60d9982-680c-4f01-9ab4-9b5d5fb05f37 Received-Content-MIC:
		 * ZMY/AoJb2JQS557MOATtc0EZdZQ=, sha1 Disposition:
		...
		// check if the returned MIC matches our outgoing MIC (sha1 of payload), warn
		// about mic mismatch
		...
		if (!outboundMic.equals(Mic.valueOf(receivedMic))) {
			log.warn("MIC mismatch, received MIC was '{}' while sent MIC was '{}'.", receivedMic, outboundMic.toString());
			return false;
		}

		// return when "clean processing state" : Disposition:
		// automatic-action/MDN-sent-automatically; processed
